## The comparison of difference: p_NO_HSC-over-p_NO_CMP_MEP--over--c_NO_HSC-over-c_NO_CMP_ME
...
param[['sampleGroup']] = 'p_NO_HSC'
param[['refGroup']] = 'c_NO_HSC'
...
param[['comparison']] = 'p_NO_HSC-over-p_NO_CMP_MEP--over--c_NO_HSC-over-c_NO_CMP_MEP'## ToAdapt
param[['name']] = 'p_NO_HSC-over-p_NO_CMP_MEP--over--c_NO_HSC-over-c_NO_CMP_MEP'## ToAdapt
param[['dataRoot']] = '/srv/gstore/projects'
param[['resultDir']] = 'p2378/DESeq2_35913_p_HU_MEP--over--p_HU_CMP_2019-04-23--14-21-06'
output = list()
output[['Name']] = 'p_NO_HSC-over-p_NO_CMP_MEP--over--c_NO_HSC-over-c_NO_CMP_MEP'## ToAdapt
...
cwd <- getwd()
setwdNew(param[['name']]) ## ToAdapt
rawData = loadCountDataset(input, param)
param$grouping = input$getColumn(param$grouping)
param$grouping2 = input$getColumn(param$grouping2)
param$sampleGroup <- c("p_NO_HSC", "p_NO_CMP_MEP") ## ToAdapt
param$refGroup <- c("c_NO_HSC", "c_NO_CMP_MEP") ## ToAdapt
...
colData <- colData[!colData$ind.n %in% c("3", "6", "7", "8"), ] ## ToAdapt otherwise it complains not full rank
...
dds = DESeq(dds)
resultsNames(dds)
res = results(dds, contrast=list("grpp_NO.cndHSC", "grpc_NO.cndHSC"))
res = as.list(res)
res$sf = sf
