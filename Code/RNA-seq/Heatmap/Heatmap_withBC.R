...
load("/srv/gstore/projects/p2378/CountQC_35760_2019-03-31--16-13-38/Count_QC/counts-jkuchrldjdcz-EzResult.RData")
rawData <- se
...
ann_colors <- list(
  "cell type"=setNames(brewer.pal(length(unique(annotation_col$`cell type`)), "Set3"),
                       sort(unique(annotation_col$`cell type`))),
  "class therapy progressed"=setNames(brewer.pal(length(unique(annotation_col$`class therapy progressed`)), "Set1"),
                                      sort(unique(annotation_col$`class therapy progressed`))),
  "patient ID"=setNames(metafolio::gg_color_hue(length(unique(annotation_col$`patient ID`))),
                        gtools::mixedsort(unique(annotation_col$`patient ID`)))
)
ann_colors$`cell type`[c("CMP", "CMP_MEP", "MEP")] <- brewer.pal(9, "YlGn")[4:6]

saveRDS(ann_colors, "ann_colors.rds")
...
pheatmap(toPlot, color=colors, clustering_method="ward.D2",
         ...
         filename="cluster-heatmap_withBC.pdf",
         height=10, width=15
         )
...
save_plot(filename="MDS_PresentGenes_withBC.pdf", p, base_height = 5)

save.image("withBC.RData")
